---
tipo: palavra
nome: storgē
aliases: [storge, astorgos, philostorgos]
status: vazio
fontes: []
lacunas: []
termos_originais: []
tags: []
ultima_revisao: null
---
## 1. Identificação

> A preencher pelo Josemar, com fonte. Ver PRD §7.

## 2. Leque semântico

> A preencher pelo Josemar, com fonte. Ver PRD §7.

## 3. Ocorrências e distribuição

> A preencher pelo Josemar, com fonte. Ver PRD §7.

## 4. Uso em contexto

> A preencher pelo Josemar, com fonte. Ver PRD §7.

## 5. Tradução

> A preencher pelo Josemar, com fonte. Ver PRD §7.

## 6. Reflexão pessoal e conexões

> A preencher pelo Josemar, com fonte. Ver PRD §7.

## Camadas pendentes
- [ ] 7. Etimologia e história da forma
- [ ] 8. Uso extrabíblico
- [ ] 9. Desenvolvimento canônico
- [ ] 10. Peso teológico
- [ ] 11. Filosofia
- [ ] 12. Distorções comuns

## Checklist anti-falácia (§8.4)
- [ ] Não tratei a origem como se fosse o sentido
- [ ] Não empilhei todos os sentidos numa ocorrência só
- [ ] Não confundi a palavra com o conceito
- [ ] Verifiquei se os termos que digo se opor chegam a se substituir em algum lugar
