...
Este projeto é acompanhado pelo Mentor de Projetos. Slug: `bibliaos`. O estado executivo vive em `.project-mentor/project.yaml` e o histórico em `.project-mentor/sessions/`. **Nunca edite esses arquivos à mão**: toda escrita passa pelo CLI `mentor`.
...
2. Rode `mentor project bibliaos --brief` e leia a última sessão em `.project-mentor/sessions/`.
...
4. Pergunte o objetivo só se não estiver claro. Depois rode `mentor start bibliaos --objective "..." --agent <claude-code|codex|antigravity|copilot>`.
...
- Ação nova: `mentor action add bibliaos --title "..."`. Concluir: `mentor action done bibliaos <act-id> --level implemented|tested`. Bloqueio: `mentor blocker add bibliaos --description "..."`.
...
2. Rode `mentor close bibliaos --from <rascunho.md>` e mostre o resultado da validação. Se falhar, mostre o erro e **não** finja sucesso.
